...
import {
    ArrowLeft, History, Clock, RefreshCw, 
    ArrowUpRight, ArrowDownLeft, SearchX
} from 'lucide-react';
...
export default function InstructorTransactionsMoney() {
    const navigate = useNavigate();
    const { transactions, isTransactionsLoading, refetchTransactions } = useInstructorWallet();
    ...
    return (
        <div className="p-4 md:p-10 max-w-6xl mx-auto space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                ...
                >
                    <RefreshCw className={cn("w-5 h-5 text-slate-500", isTransactionsLoading && "animate-spin")} />
                </button>
            </div>
                ...
                {isTransactionsLoading ? (
                    ...
                    <div className="p-4 md:p-8 space-y-3">
                        {transactions.map((tx: any) => {
                            const isAdd = tx.type === 'add' || tx.type === 'purchase' || tx.type === 'sale' || tx.type === 'bonus';
