    ...
    const toastId = toast(
      <div className="flex flex-col gap-3">
        <div className="flex items-center gap-2 text-gray-900">
          ...
          <p className="font-bold">Delete this question?</p>
        </div>
        <div className="flex justify-end gap-2 mt-2">
          ...
          <Button 
            ...
            onClick={async () => { 
              ...
              try {
                await deleteQuizMutation.mutateAsync({ quizId, courseId: id });
      ...
      <header className="sticky top-0 z-50 bg-white/80 backdrop-blur-xl border-b border-gray-100 px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div className="flex items-center gap-4">
            ...
            <div>
              <h1 className="text-xl font-bold text-gray-900 leading-none mb-1">Quiz Management</h1>
          ...
          <Button
            onClick={() => {
              setEditingQuizId(null);
              setFormData({ question: '', option_a: '', option_b: '', option_c: '', option_d: '', correct_answer: 'option_a', points: 10 });
              setIsAdding(true);
            }}
                  ...
                  {[
                    ...
                    <div key={opt.id} className="space-y-2">
                      ...
                      <div className="relative">
                        ...
                        />
                        <button
                          onClick={() => setFormData({ ...formData, correct_answer: `option_${opt.id}` })}
                          className={`absolute left-2 top-1/2 -translate-y-1/2 w-8 h-8 rounded-lg flex items-center justify-center font-black text-xs transition-all ${formData.correct_answer === `option_${opt.id}` ? 'bg-emerald-500 text-white shadow-lg' : 'bg-white text-gray-300'
                            }`}
                        >
                          {opt.id.toUpperCase()}
                        </button>
              ...
              >
                {editingQuizId === quiz.id ? (
                  <div className="space-y-8">
                    <h3 className="text-sm font-black text-orange-600 uppercase tracking-widest">Edit Question</h3>
                    <div className="space-y-6">
                      <div className="space-y-2">
                        <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">Question Text</label>
                        <Input
                      ...
                      <div className="flex items-center justify-between pt-6 border-t border-gray-50">
                        ...
                        <div className="flex gap-4">
                          ...
                          <Button
                            ...
                            className="bg-orange-600 hover:bg-orange-700 text-white rounded-xl font-black h-12 px-10"
                          >
                            {updateQuizMutation.isPending ? 'UPDATING...' : 'UPDATE QUESTION'}
                          </Button>
                        </div>
                      </div>
                    </div>
                  ...
                  <>
                    ...
                    <div className="flex items-start justify-between mb-6">
                      ...
                      <div className="flex items-center gap-1 shrink-0">
                        <button onClick={() => handleEditClick(quiz)} className="w-9 h-9 rounded-xl flex items-center justify-center text-gray-300 hover:text-gray-500 hover:bg-gray-50 transition-all">
                          <Edit2 size={16} />
                        </button>
                        <button onClick={() => handleDeleteClick(quiz.id)} disabled={deleteQuizMutation.isPending} className="w-9 h-9 rounded-xl flex items-center justify-center text-gray-300 hover:text-red-400 hover:bg-red-50 transition-all disabled:opacity-50">
                          <Trash2 size={16} />
                        </button>
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pl-[4.75rem]">
                      {[
                        { label: 'option_a', display: 'A', value: quiz.option_a },
                        { label: 'option_b', display: 'B', value: quiz.option_b },
                        { label: 'option_c', display: 'C', value: quiz.option_c },
                        { label: 'option_d', display: 'D', value: quiz.option_d },
                      ].map((opt) => {
                        const isCorrect = quiz.correct_answer === opt.label;
                        return (
                          <div
                            key={opt.label}
                            className={`p-4 rounded-2xl flex items-center gap-3 border transition-all ${isCorrect
                              ? 'bg-emerald-50 border-emerald-100'
                              : 'bg-gray-50/60 border-gray-100'
                              }`}
                          >
                            {/* Letter Circle */}
                            <div className={`w-8 h-8 rounded-full flex items-center justify-center font-black text-xs shrink-0 ${isCorrect ? 'bg-emerald-500 text-white' : 'bg-white text-gray-400 border border-gray-200'
                              }`}>
                              {opt.display}
                            </div>
                            <span className={`text-sm font-semibold leading-snug ${isCorrect ? 'text-emerald-700' : 'text-gray-600'
                              }`}>
                              {opt.value}
                            </span>
                          </div>
