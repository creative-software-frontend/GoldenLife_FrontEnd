...
interface InstructorOtpResponse {
  success: boolean;
  user_id?: number;
  message?: string;
  token?: string;
...
export const useInstructorOtp = () => {
  ...
  /**
   * Send OTP to instructor's email or mobile
   */
  const sendOtp = useCallback(async (credential: string, type: 'email' | 'mobile'): Promise<InstructorOtpResponse> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      console.log('🟢 [useInstructorOtp] Full Response:', {
  ...
  /**
   * Verify OTP and login instructor
   */
  const verifyOtp = useCallback(async (otpCode: string): Promise<InstructorOtpResponse> => {
    if (!userId) {
    ...
    try {
      ...
      console.log('🟢 [useInstructorOtp] OTP verified successfully:', response.data);
      ...
      if (response.data.success && response.data.token) {
        ...
        console.log('✅ [useInstructorOtp] Session stored successfully');
      }

      return response.data;
    } catch (err: any) {
      console.error('❌ [useInstructorOtp] Failed to verify OTP:', err);
   ...
   */
  const loginWithOtp = useCallback(async (mobile: string, otpCode: string): Promise<InstructorOtpResponse> => {
    setIsLoading(true);
    setError(null);
    ...
    try {
      ...
      console.log('🟢 [useInstructorOtp] Direct OTP login successful:', response.data);
      ...
      console.error('❌ [useInstructorOtp] Failed direct OTP login:', err);
