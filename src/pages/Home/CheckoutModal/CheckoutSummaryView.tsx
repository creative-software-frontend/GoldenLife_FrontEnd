...
const CheckoutSummaryView = ({
    ...
    const { changeCheckoutModal, toggleClicked, triggerWalletUpdate, checkoutSellerId } = useModalStore();
    const {
    ...
    const { cartItems: allCartItems, removeItem, clearCart } = useCartStore();

    const cartItems = checkoutSellerId
        ? allCartItems.filter((item: any) => (item.seller_id || item.vendor_id || 'default_seller') === checkoutSellerId)
        : allCartItems;
            ...
            {cartItems.length > 0 && (
                <div className="space-y-4">
                    {cartItems.map((item: any, idx: number) => {
                        ...
                        const offerPrice = Number(item.offer_price) || 0;
                        const regularPrice = Number(item.regular_price) || Number(item.price) || 0;
                        const activePrice = offerPrice;
                        const origPrice = regularPrice;
                        const itemSubTotal = activePrice * qty;
                        const imageUrl = item.image || item.product_image || item.thumbnail;

                        return (
                            <div key={idx} className="bg-white p-4 rounded-2xl border border-slate-200 shadow-sm flex gap-4 relative">
                                {/* Left: Product Image */}
                                    ...
                                    <div className="flex justify-between items-start gap-2">
                                        <div>
                                            <h3 className="text-[16px] font-bold text-slate-900 leading-tight">
                                                {item.product_title_english || item.name || "Unknown Item"}
                                            </h3>
                                        ...
                                        <button onClick={() => handleRemoveItem(item.id)} className="text-slate-300 hover:text-red-500 transition-colors p-1 -mt-1 -mr-1">
